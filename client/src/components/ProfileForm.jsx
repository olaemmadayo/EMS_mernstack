...
const ProfileForm = ({ initialData, onSuccess }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="card p-5 sm:p-6 mb-6">
      <h2 className="text-base font-medium text-slate-900 mb-6 pb-4 broder-b border-slate-100 flex items-center gap-2">
      ...
      {message && (
        <div className="bg-emerald-50 text-emrald-700 p-4 rounded-xl text-sm border border-emrald-200 mb-6 flex items-start gap-3">
          <div className="w-1.5 h-1.5 rounded-full bg-emrals-500 mt-1.5 shrink-0" />
          {message}
        </div>
      )}
      <div className="space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 ">
          <div className="">
            ...
            <input
              disabled
              valued={`${initialData.firstName} ${initialData.lastName}`}
